...
def is_manager(user):
    """
    【店長・エンジニア 権限判定】
    - 管理者権限(is_staff)にチェックがある ＝ 店長
    - スーパーユーザー権限(is_superuser)にチェックがある ＝ エンジニア
    - demo_managerグループに属している ＝ デモ店長
    """
    if not user.is_authenticated:
        return False
        
    return (
        user.is_staff           # 画像の「管理者権限」チェック
        or user.is_superuser    # 画像の「スーパーユーザー権限」チェック
        or user.groups.filter(name="demo_manager").exists()
    )
...
def is_staff_member(user):
    ...
    【全従業員 権限判定】
    - 店長以上の権限を持っている
    - または、ログインしている（チェックなしの一般スタッフ）
    """
    if not user.is_authenticated:
        return False

    # 一般スタッフは「チェックなし」なので、ログインさえしていればOKとする
    # (もしdemo_staffを分けたい場合もここで判定)
    return True
...
class EstimateListView(LoginRequiredMixin, ListView):
    """
    見積の一覧を表示するView
    👉 ログインしていない人はアクセス不可（ログイン画面へリダイレクト）
    """
    model = Estimate
...
# ==========================================
@method_decorator(stop_demo_user, name='dispatch') # 早期ガードをデコレータに統一（403エラーを送出)
class EstimateStatusUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        """詳細画面（管理画面）からのステータス更新を受け付ける"""
        # 🚀 ログ出力（RenderのログでView到達を確認するため、実務的なloggerを使用）
        logger.info("DEBUG POST HIT")
        ...
        if not estimate:
            # 🌟 ログに詳細を残す
            logger.error(f"Estimate not found: pk={pk}")
        ...
        try:
            ...
            # 🛡️ get() でデータが見つからなかった場合の明確なエラー通知（ログにも詳細を記録）
            logger.error(f"Target status not found. Input: {quick_status_name or status_id}")
            messages.error(request, f"エラー：マスタに指定のステータスが存在しません。")
        except Exception as e:
            # 🛡️ TypeError や DBエラーなど、すべての予期せぬエラーをここでキャッチして500を回避
            # 🌟 logger.exception を使うと、エラーが起きた行数などの詳細がログに残る
            logger.exception(f"System error in EstimateStatusUpdateView: {str(e)}")
